...
#include "stdafx.h"
#include "quick_x_cclabelchange.h"
#include <io.h>
#include <stdlib.h>
#include <stdio.h>
#include <fcntl.h>
    ...
    int       nCmdShow)
{
    ...
    return Quick_x_cclabelchangePlayer::createAndRun();
}

int Quick_x_cclabelchangePlayer::createAndRun(void)
{
    Quick_x_cclabelchangePlayer *host = Quick_x_cclabelchangePlayer::sharedInstance();
    int ret = host->run();
    Quick_x_cclabelchangePlayer::purgeSharedInstance();
    return ret;
}

Quick_x_cclabelchangePlayer *Quick_x_cclabelchangePlayer::s_sharedInstance = NULL;

Quick_x_cclabelchangePlayer *Quick_x_cclabelchangePlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new Quick_x_cclabelchangePlayer();
    }
    return s_sharedInstance;
}

void Quick_x_cclabelchangePlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
Quick_x_cclabelchangePlayer::Quick_x_cclabelchangePlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int Quick_x_cclabelchangePlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "quick_x_cclabelchange-player");
...
void Quick_x_cclabelchangePlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void Quick_x_cclabelchangePlayer::createViewMenu(void)
{
...
void Quick_x_cclabelchangePlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void Quick_x_cclabelchangePlayer::updateWindowTitle(void)
{
}

void Quick_x_cclabelchangePlayer::relaunch(void)
{
...
void Quick_x_cclabelchangePlayer::writeDebugLog(const char *log)
{
...
// menu callback
void Quick_x_cclabelchangePlayer::onFileOpenProject(void)
{
...
void Quick_x_cclabelchangePlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"Quick_x_cclabelchangePlayer");
...
void Quick_x_cclabelchangePlayer::onFileProjectConfig(void)
{
...
void Quick_x_cclabelchangePlayer::onFileRelaunch(void)
{
    relaunch();
}

void Quick_x_cclabelchangePlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void Quick_x_cclabelchangePlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void Quick_x_cclabelchangePlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void Quick_x_cclabelchangePlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void Quick_x_cclabelchangePlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT Quick_x_cclabelchangePlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    Quick_x_cclabelchangePlayer *host = Quick_x_cclabelchangePlayer::sharedInstance();
...
INT_PTR CALLBACK Quick_x_cclabelchangePlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
